...
Twinkle.warn.messages = {
	levels: {
		...
		[wgULS('非能接受且违反方针或指引的单方面行为或操作', '非能接受且違反方針或指引的單方面行為或操作')]: {
			'uw-notcensored': {
				level1: {
	...
	singlenotice: {
		...
		'uw-chinese': {
			label: wgULS('不是以中文进行沟通', '不是以中文進行溝通'),
			summary: wgULS('提醒：不是以中文进行沟通', '提醒：不是以中文進行溝通')
		},
		'uw-coi': {
		...
		'uw-uaa': {
			label: wgULS('向需要管理员注意的用户名报告的用户名并不违反方针', '向需要管理員注意的用戶名報告的用戶名稱並不違反方針'),
			summary: wgULS('提醒：向需要管理员注意的用户名报告的用户名并不违反方针', '提醒：向需要管理員注意的用戶名報告的用戶名稱並不違反方針')
		},
		'uw-warn': {
