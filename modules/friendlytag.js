...
Twinkle.tag.article.tagList = {
	[wgULS('清理和维护模板', '清理和維護模板')]: {
		...
		[wgULS('可能多余的内容', '可能多餘的內容')]: [
			...
			{ tag: 'Non-free', description: wgULS('', '可能過多或不當地使用了受版權保護的文字、圖像或/)及多媒體檔案') }
		],
		[wgULS('结构和导言', '結構和導言')]: [
	...
	[wgULS('具体内容问题', '具體內容問題')]: {
		[wgULS('语言', '語言')]: [
			...
			{ tag: 'Rough translation', description: wgULS('翻译质量不佳', '翻譯品質不佳') }
		],
		[wgULS('链接', '連結')]: [
...
Twinkle.tag.redirectList = {
	['常用模板']: [
		...
		{ tag: '模板重定向', description: wgULS('指向模板的重定向页', '指向模板的重新導向頁面') },
			...
			subgroup: [
				{
					name: 'altLangFrom',
					type: 'input',
					label: wgULS('本重定向的语言（可选）', '本重新導向的語言（可選）'),
					tooltip: wgULS('输入重定向名称所使用语言的ISO 639代码，例如en代表英语，代码可参见 Template:ISO_639_name', '輸入重新導向名稱所使用語言的ISO 639代碼，例如en代表英語，代碼可參見 Template:ISO_639_name')
				}
			]
		},
			...
			subgroup: [
				{
					...
					tooltip: wgULS('使用ISO 639代码，可参见 Template:ISO_639_name', '使用ISO 639代碼，可參見 Template:ISO_639_name')
				},
				{
	...
	[wgULS('鲜用模板', '鮮用模板')]: [
		...
		{ tag: wgULS('重定向模板用重定向', '重新導向模板用重定向'), description: wgULS('导向至重定向模板', '導向至重新導向模板') },
